...
function injectVocabularyHTML() {
    ...
    container.innerHTML = `
        <div id="vocabulary-categories" class="container mx-auto px-4">
            ...
            <div id="vocabulary-category-grid" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4"></div>
        </div>
...
function createVocabularyCategories() {
    ...
    Object.keys(vocabModuleData).forEach(category => {
        ...
        // 모바일 한 화면에 여러 개가 들어오도록 더 작고 컴팩트한 카드로 조정
        card.className = 'bg-white rounded-lg shadow-sm p-3 hover:shadow-md transition-all cursor-pointer border border-gray-100 transform hover:-translate-y-1';
        card.innerHTML = `
            <div class="flex flex-col items-center">
                <div class="w-12 h-12 rounded-full bg-blue-50 flex items-center justify-center mb-2">
                    <i class="${categoryIcons[category] || 'fas fa-book'} text-xl text-blue-500"></i>
                </div>
                <h3 class="text-xs font-bold text-gray-800 mb-1 text-center break-keep leading-snug">${category}</h3>
                <span class="text-[10px] text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">${vocabModuleData[category].length} 단어</span>
            </div>
        `;
